...
uint16_t Modbus_Write_Read_function(uint8_t address_,uint16_t reg_WRITE_,uint16_t reg_READ_,uint16_t msgWrite)
{
  //Serial.println("Escritura 1 ");
  Modbus_Write_function(address_,reg_WRITE_,msgWrite);
  delay(20); 
  //Serial.println("Escritura 2 ");
  Modbus_Write_function(address_,reg_WRITE_,msgWrite);
  delay(20);
  
  uint16_t valor2= Modbus_Read_function(address_,reg_READ_);
  //Serial.println("Lectura 1 ");
  //Serial.println(valor2); 
  delay(20);  

  uint16_t Stable_Valor= Modbus_Read_function(address_,reg_READ_);
  //Serial.println("Lectura 2 ");
  //Serial.println(Stable_Valor); 
  delay(20);  
  return Stable_Valor;
}
uint16_t Modbus_Read_function(uint8_t address2,uint16_t reg2)
{
    ...
    if (Serial2.available() > 0) 
    {
        ...
        */     
        ...
        //Serial.println(valor_total,HEX); 
        //Serial.println(valor_total,DEC);
        return valor_total;   
...
void SerialModbus_initialize(int Serial_input,uint16_t Baud)
{
    ...
    if(Serial_input==2)
    {
      Serial2.begin(Baud, SERIAL_8E1,16, 17); //esp32
      //Serial2.begin(Baud,SERIAL_8E1);  //arduino       
    }
    /*
